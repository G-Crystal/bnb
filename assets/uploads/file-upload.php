<?php
 ...
 function validateAndSave($file) {
		...
		)
	 {
		...
		$file_name = explode('.', preg_replace('/[^\w\.\- ]/', '', $file['name']) );
		//Add time in file name to prevent duplicate and replacement		
		$save_path = $file_name[0].'-'.time().'.'.$file_name[1];
