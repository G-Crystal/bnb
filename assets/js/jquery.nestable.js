...
;(function($, window, document, undefined)
{
    ...
    Plugin.prototype = {
        ...
        init: function()
        {
            ...
            list.el.on('click', '.dd2-content, .dd-item a[data-action]', function(e){
                e.preventDefault();
                var $this = $(this);
                var $box = $this.closest('.dd-item');
                ...
                if($action == 'collapse') {
                    ...
                    list.slide($box,event_name,$this);
                }
                    ...
                    list.remove_list($box);
                }
                
            });

            list.el.on('keyup', '.dd-body input[data-name]', function(e){
                var $this = $(this),
                    $box = $this.closest('.dd-item')
                    data = $box.find('input[data-name]');

                $box.find('.title').first().html(data.val());              
            });

            list.el.on('keyup', '.dd-body input[data-icon]', function(e){
                var $this = $(this),
                    $box = $this.closest('.dd-item'),
                    $data = $box.find('input[data-icon]'),
                    $icon = $box.find(ace.vars['.icon']).eq(0);
                if($icon){
                    $icon_classes = $icon.attr('class').match(/fa\-(.*)/);
                    if($icon_classes){
                        $icon_class = $icon_classes[0].split(' ')[0];
                        $icon.removeClass($icon_class).addClass($data.val());
                    }else{
                        $icon.addClass('fa-');
                    }
                }
        ...
        slide: function(box, type, button){
            this.$box = $(box);
            var $box = this.$box;
            var $body = $box.find('.dd-body').eq(0);
            var $icon = null;

            var event_name = typeof type !== 'undefined' ? type : ($box.hasClass('collapsed') ? 'show' : 'hide');
            var event_complete_name = event_name == 'show' ? 'shown' : 'hidden';
            if(typeof button === 'undefined') {
                button = $box.find('> .action-buttons a[data-action=collapse]').eq(0);
                    ...
                    items.each(function()
                    {
                        ...
                        inputs.each(function(){
                            ...
                            $.each(data.data(), function( k,v ){
                                input_data[k] = data.val();
                            })
                            item = $.extend(item, input_data);
                        })
        ...
        {
            list  = this;
            level = list.el.find(list.options.listNodeName);
            items = level.children(list.options.itemNodeName);
            $max = 1;
        ...
        disableLink: function(li)
        {
            var widget = li.find('.dd-body').first();
            if( widget ){
                var link = widget.find('input[data-link]');
                link.attr('readonly',true);
            }
        },

        enableLink: function(li)
        {
            var widget = li.find('.dd-body').first();
            if( widget ){
                var link = widget.find('input[data-link]');
